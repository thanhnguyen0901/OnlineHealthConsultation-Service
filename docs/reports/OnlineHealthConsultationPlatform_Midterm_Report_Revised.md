...
2. Phân tích hệ thống
   - 2.1. Usecase tổng quan toàn hệ thống
   - 2.2. Usecase Guest tra cứu bác sĩ - UC01
   - 2.3. Usecase đăng ký / đăng nhập / phân quyền - UC02
   - 2.4. Usecase Patient đặt lịch tư vấn - UC03
...
3. Định nghĩa yêu cầu chất lượng phần mềm
   - 3.1. Mục tiêu
...
NOTE: Hình 1 không đặt tại phần Giới thiệu đề tài. Hình 1 sẽ được chèn tại mục **Chương I, 3.1. Quy trình nghiệp vụ của hệ thống tư vấn sức khỏe trực tuyến** để minh họa luồng nghiệp vụ tổng quan sau khi đã mô tả các quy trình chính.
...
TODO: Cần insert **Hình 1 — Tổng quan quy trình tư vấn sức khỏe trực tuyến** tại đây.

NOTE: Hình 1 nên là business process / flowchart tổng quan, đặt ngay sau danh sách các quy trình nghiệp vụ chính ở mục 3.1. Hình này dùng để cho người đọc nhìn nhanh luồng tổng thể từ Guest User tra cứu thông tin, Patient đăng ký/đăng nhập, đặt lịch/gửi câu hỏi, Doctor tư vấn/ghi kết quả, Administrator quản lý hệ thống và các dịch vụ hỗ trợ như Notification, Video, File Storage.
...
TODO: Cần insert hình wireframe/prototype nếu báo cáo yêu cầu.
...
TODO: Cần insert Hình 2 — Use case tổng quan toàn hệ thống.

- **Mục tiêu:**  
  Mô tả tổng quan các nhóm chức năng chính của hệ thống và mối quan hệ giữa các actor với nền tảng tư vấn sức khỏe trực tuyến.

- **Actor:**  
  Guest User, Patient, Doctor, Administrator.

- **Actor phụ / hệ thống bên ngoài:**  
  Notification Service, Video Communication Service, File Storage Service.

- **Mô tả:**  
  Guest User có thể tra cứu thông tin công khai về chuyên khoa và bác sĩ. Patient có thể đăng ký, đăng nhập, quản lý hồ sơ sức khỏe, đặt lịch tư vấn, gửi câu hỏi, tham gia phiên tư vấn, xem kết quả, xem đơn thuốc, đánh giá và nhận thông báo. Doctor có thể quản lý hồ sơ, xem và phản hồi câu hỏi, quản lý lịch, tư vấn trực tuyến, ghi kết quả và cấp đơn thuốc. Administrator có thể quản lý người dùng, chuyên khoa, lịch hẹn, nội dung cần kiểm duyệt và dashboard thống kê.

- **Phạm vi:**  
  Usecase tổng quan chỉ thể hiện các nhóm chức năng chính ở mức hệ thống. Các usecase chi tiết theo từng nhóm actor được trình bày ở các mục tiếp theo.

### 2.2. Usecase Guest tra cứu bác sĩ - UC01

TODO: Cần insert Hình 3 — Use case Guest User.

- **Mục tiêu:**  
  Cho phép Guest User truy cập thông tin công khai của hệ thống, xem chuyên khoa, tìm kiếm bác sĩ, xem hồ sơ công khai của bác sĩ và được chuyển đến đăng nhập/đăng ký khi muốn sử dụng chức năng yêu cầu xác thực.
...
- **Luồng sự kiện chính:**
  ...
  2. Hệ thống hiển thị thông tin giới thiệu nền tảng và các chức năng công khai.
  3. Guest User xem danh sách chuyên khoa hoặc danh sách bác sĩ.
  4. Guest User tìm kiếm bác sĩ theo chuyên khoa hoặc từ khóa.
  5. Hệ thống hiển thị danh sách bác sĩ phù hợp.
  6. Guest User chọn một bác sĩ để xem hồ sơ công khai.
  7. Khi Guest User chọn đặt lịch hoặc gửi câu hỏi, hệ thống chuyển đến trang đăng nhập hoặc đăng ký.
...
- **Biểu đồ hoạt động:**  
  TODO: Cần insert Hình 7 — Biểu đồ hoạt động Guest tra cứu bác sĩ.

### 2.3. Usecase đăng ký / đăng nhập / phân quyền - UC02

- **Mục tiêu:**  
  Cho phép người dùng đăng ký tài khoản, đăng nhập, đăng xuất và được điều hướng đến khu vực chức năng phù hợp theo vai trò.
...
- **Luồng sự kiện chính:**
  ...
  3. Guest User nhập thông tin đăng ký.
  4. Hệ thống kiểm tra dữ liệu và tạo tài khoản Patient nếu hợp lệ.
  ...
  8. Người dùng được chuyển đến khu vực phù hợp theo vai trò.
  9. Người dùng chọn đăng xuất để kết thúc phiên làm việc.
...
- **Biểu đồ hoạt động:**  
  TODO: Cần insert Hình 8 — Biểu đồ hoạt động đăng ký / đăng nhập / phân quyền.

### 2.4. Usecase Patient đặt lịch tư vấn - UC03

TODO: Cần insert Hình 4 — Use case Patient.

- **Mục tiêu:**  
  Cho phép Patient tìm kiếm bác sĩ, xem hồ sơ bác sĩ, chọn thời gian tư vấn, đặt lịch hẹn và nhận thông báo xác nhận hoặc nhắc lịch.
...
- **Luồng sự kiện chính:**
  ...
  2. Patient tìm kiếm bác sĩ theo chuyên khoa hoặc từ khóa.
  3. Patient xem chi tiết hồ sơ bác sĩ.
  4. Patient chọn ngày và khung giờ tư vấn còn khả dụng.
  5. Patient nhập lý do tư vấn và thông tin cần thiết.
  6. Hệ thống kiểm tra dữ liệu đặt lịch.
  7. Hệ thống kiểm tra tính khả dụng của khung giờ.
  8. Hệ thống tạo lịch hẹn với trạng thái phù hợp.
  9. Patient xem danh sách lịch hẹn sắp tới.
  10. Hệ thống gửi thông báo xác nhận hoặc nhắc lịch nếu được cấu hình.
...
- **Biểu đồ hoạt động:**  
  TODO: Cần insert Hình 9 — Biểu đồ hoạt động Patient đặt lịch tư vấn.

### 2.5. Usecase Patient gửi câu hỏi và Doctor phản hồi - UC04

- **Mục tiêu:**  
  Cho phép Patient gửi câu hỏi sức khỏe, Doctor xem và phản hồi câu hỏi, hệ thống cập nhật trạng thái và thông báo cho Patient khi có phản hồi.
...
- **Luồng sự kiện chính:**
  1. Patient mở chức năng gửi câu hỏi sức khỏe.
  2. Patient nhập tiêu đề và nội dung câu hỏi.
  3. Hệ thống kiểm tra dữ liệu câu hỏi.
  4. Hệ thống lưu câu hỏi với trạng thái chờ xử lý.
  5. Doctor xem danh sách câu hỏi được phân công hoặc có thể xử lý.
  6. Doctor mở chi tiết câu hỏi.
  7. Doctor nhập nội dung phản hồi.
  8. Hệ thống lưu phản hồi của Doctor.
  9. Hệ thống cập nhật trạng thái câu hỏi.
  10. Hệ thống gửi thông báo cho Patient.
  11. Patient xem phản hồi của bác sĩ.
...
- **Biểu đồ hoạt động:**  
  TODO: Cần insert Hình 10 — Biểu đồ hoạt động Patient gửi câu hỏi và Doctor phản hồi.

### 2.6. Usecase Doctor tư vấn, ghi kết quả và đơn thuốc - UC05

TODO: Cần insert Hình 5 — Use case Doctor.

- **Mục tiêu:**  
  Cho phép Doctor bắt đầu phiên tư vấn, trao đổi với Patient qua chat hoặc video, ghi nhận kết quả tư vấn và tạo đơn thuốc điện tử cơ bản.

- **Actor:**  
  Doctor, Patient.

- **Actor phụ:**  
  Video Communication Service, File Storage Service.
...
- **Luồng sự kiện chính:**
  1. Doctor mở lịch hẹn cần tư vấn.
  2. Doctor bắt đầu phiên tư vấn.
  3. Hệ thống kiểm tra trạng thái lịch hẹn và quyền của Doctor.
  4. Patient tham gia phiên tư vấn theo lịch hẹn của mình.
  5. Hệ thống kiểm tra quyền của Patient.
  6. Doctor và Patient trao đổi qua chat hoặc video.
  7. Doctor ghi nhận kết quả tư vấn.
  8. Doctor tạo đơn thuốc điện tử nếu cần.
  9. Hệ thống lưu kết quả tư vấn và đơn thuốc.
  10. Patient xem tóm tắt tư vấn và đơn thuốc sau phiên tư vấn.
...
- **Biểu đồ hoạt động:**  
  TODO: Cần insert Hình 11 — Biểu đồ hoạt động Doctor tư vấn, ghi kết quả và đơn thuốc.

### 2.7. Usecase Administrator quản lý hệ thống và dashboard - UC06

TODO: Cần insert Hình 6 — Use case Administrator.

- **Mục tiêu:**  
  Cho phép Administrator quản lý tài khoản, chuyên khoa, lịch hẹn, kiểm duyệt nội dung và theo dõi hoạt động hệ thống thông qua dashboard/reporting.
...
- **Luồng sự kiện chính:**
  ...
  2. Administrator quản lý tài khoản bác sĩ.
  3. Administrator quản lý tài khoản bệnh nhân.
  4. Administrator quản lý danh mục chuyên khoa.
  5. Administrator theo dõi và quản lý lịch hẹn.
  6. Administrator kiểm duyệt nội dung tư vấn, phản hồi hoặc đánh giá khi cần.
  7. Administrator xem dashboard thống kê hệ thống.
  8. Administrator theo dõi người dùng đang hoạt động và số lượng phiên tư vấn.
...
- **Biểu đồ hoạt động:**  
  TODO: Cần insert Hình 12 — Biểu đồ hoạt động Administrator quản lý hệ thống và dashboard.
...
Việc định nghĩa yêu cầu chất lượng phần mềm nhằm bảo đảm hệ thống hoạt động đúng theo yêu cầu, an toàn, dễ sử dụng và có thể kiểm thử. Các yêu cầu chất lượng được xác định từ góc nhìn nghiệp vụ, vận hành và phát triển để làm cơ sở thiết kế test case ở giai đoạn tiếp theo.

Do hệ thống xử lý dữ liệu cá nhân và dữ liệu sức khỏe, báo cáo chú trọng các tiêu chí như bảo mật, quyền riêng tư, phân quyền theo vai trò, ownership dữ liệu, validation dữ liệu đầu vào, kiểm soát trạng thái lịch hẹn và tính đúng đắn của phiên tư vấn trực tuyến.
...
TODO: Cần insert Hình 13 — Kiến trúc hệ thống.
...
TODO: Cần insert hình giao diện/form minh họa nếu báo cáo yêu cầu.
...
TODO: Cần insert Hình 14 — ERD / Database diagram.

#### 4.3.1. Nhóm bảng chính
...
#### 4.3.2. Từ điển cơ sở dữ liệu rút gọn
...
1. Tài liệu yêu cầu phần mềm (SRS): *Online Health Consultation Platform*, nhóm thực hiện, 2026.
...
8. Roger S. Pressman, *Software Engineering: A Practitioner's Approach*, 8th edition, McGraw-Hill, 2014.

---

> **Ghi chú:** Đây là báo cáo giữa kỳ. Nội dung bao gồm Chương I (Tổng quan) và Chương II (Phân tích & Thiết kế hệ thống). Chương III (Triển khai & Kiểm thử), Chương IV (Kết luận) và toàn bộ kết quả kiểm thử thực tế sẽ được trình bày đầy đủ trong báo cáo cuối kỳ.
